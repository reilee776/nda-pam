...
int parse_JsonResponse_from_login_request(const char *res_doc, st_user_login_result *plogin_result)
{
        ...
        if (!json_object_object_get_ex(parse_json, "contentSimpleType", &contentSimpleType))
        {

                json_object_put(parse_json);
                return EXCEPTION;
        }

        plogin_result->contentSimpleType = strdup(contentSimpleType);
        ...
        if (json_object_object_get_ex(content, PAM_JSON_KEY_NM_CONTENTSIMPLETYPE, &contentSimpleType))
        {
                ...
                if (strcmp(contentSimpleTypeStr, PAM_JSON_KEY_VALUE_RET_REQREG) == 0) // need to registry message
                {
                        json_object_put(parsed_json);
                        return HI_AUTH_RET_REQ_REGOTP;
                }
        }
...
int requestHiwareAuthToApiServer(const char *username, const char *passwd, const char *agt_auth_no, struct st_hiauth_hiware_login_result *result)
{
        ...
        char *strConfLocalIp = get_value_from_inf(g_sConfFilePath, "AGENT_INFO", "PAM_LOCAL_IP");
        ...
        if (retval && strlen(response.m_data) > 0)
        {
                ...
                if (retval != 0)
                {
                        ...
                        if (retval == HI_AUTH_RET_REQ_REGOTP)
                        {
